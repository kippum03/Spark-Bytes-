...
export const create_event = async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Preparing tags data for Prisma, handling case when tags are undefined
    let tagsData = {};
    ...
    let locationData = {};
    if (location && location.Address && location.floor && location.room) {
      locationData = {
        create: {
    ...
    if (photos && photos.length > 0) {
      photosData = photos.map((photoBase64) => {
    ...
    const newEvent = await prisma.event.create({
      data: {
        ...
        ...(photosData.length > 0 && { photos: { create: photosData } }),
      },
    });
